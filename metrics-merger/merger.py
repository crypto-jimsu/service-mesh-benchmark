...
def get_completed_runs(p, mesh):
    s = get_series(p,
            'wrk2_benchmark_progress{job="%s",status="done"}'%(mesh,))
...
prometheus_url = environ['PROMETHEUS_URL']
pgw_url = environ['PushGateway_URL']
past_days=7
...
for mesh in ["bare-metal", "svcmesh-linkerd", "svcmesh-istio"]:
    ...
    dg, dpercs, druns = create_summary_gauge(p, mesh, r, detailed=True, past_days=past_days)
    print("%s: %d runs with %d percentiles (coarse)" % (mesh, runs, percs))
    print("%s: %d runs with %d percentiles (detailed)" % (mesh, druns, dpercs))
    push_to_gateway(pgw_url, job=mesh, grouping_key={"instance":"emojivoto"}, registry=r)
